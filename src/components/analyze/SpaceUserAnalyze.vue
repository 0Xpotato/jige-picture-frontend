<template>
  <div class="space-user-analyze">
    <a-card title="空间图片用户分析">
      <v-chart :loading="loading" :option="options" style="height: 320px; max-width: 100%" />
      <template #extra>
        <a-space>
          <a-segmented v-model:value="timeDimension" :options="timeDimensionOptions" />
          <a-input-search enter-button="搜索用户" placeholder="请输入用户 id" @search="doSearch" />
        </a-space>
      </template>
    </a-card>
  </div>
...
import { getSpaceUserAnalyzeUsingPost } from '@/api/spaceAanalyzeController.ts'
